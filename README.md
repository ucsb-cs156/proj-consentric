...
* Prod: <https://consentric.dokku-00.cs.ucsb.edu>
* QA: <https://consentric-qa.dokku-00.cs.ucsb.edu>
...
By convention, we are putting Integration tests (the ones that run with Playwright) under the package `src/test/java/edu/ucsb/cs156/consentric/web`.
...
```
mvn pitest:mutationCoverage -DtargetClasses=edu.ucsb.cs156.consentric.controllers.SomeController
```

To run pitest on just one package, use:

```
mvn pitest:mutationCoverage -DtargetClasses=edu.ucsb.cs156.consentric.controllers.\*
```
